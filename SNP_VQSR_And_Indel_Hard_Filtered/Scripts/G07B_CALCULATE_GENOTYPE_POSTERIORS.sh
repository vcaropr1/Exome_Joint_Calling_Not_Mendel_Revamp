...
CMD=$CMD' --variant '$CORE_PATH'/'$PROJECT'/MULTI_SAMPLE/'$PREFIX'.HC.VQSR.SNP.HF.INDEL.vcf'
CMD=$CMD' --supporting '$P3_1KG
CMD=$CMD' --supporting '$ExAC
CMD=$CMD' --disable_auto_index_creation_and_locking_when_reading_rods'
CMD=$CMD' -et NO_ET'
CMD=$CMD' -K '$KEY
CMD=$CMD' -o '$CORE_PATH'/'$PROJECT'/MULTI_SAMPLE/'$PREFIX'.BEDsuperset.VQSR.HF.1KG.ExAC3.REFINED.temp.vcf'
